...
public:
  SimilarNeighborhoodRule(WorkingCopyInstance& inst, double c, bool conserveMultipleSolutions) : FPTReductionRule(inst, "Similar Neighborhood Rule", conserveMultipleSolutions), _c(c), _delta(inst.getGraph()), _upperBoundCrossRef(inst.getGraph(), -1), _upperBound(_upperBoundCrossRef) {
  }
  void apply();
private:
  void init();
  ...
  double computeDelta(WorkingCopyGraph::NodeMap<bool>& neighborhood, WorkingCopyGraph::NodeMap<bool>& neighborhoodOpp, WorkingCopyGraph::Node& node);
