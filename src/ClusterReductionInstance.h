...
public:
    ClusterReductionInstance(ClusterEditingInstance* inst, std::bitset<NUMBER_OF_REDUCTION_RULES> active, double multiplicativeFactor, bool conserveMultipleSolutions) : _lastSuccessRule(-1), _currentRule(0), _instance(inst), _active(active), _multiplicativeFactor(multiplicativeFactor), _conserveMultipleSolutions(conserveMultipleSolutions) {
    }
